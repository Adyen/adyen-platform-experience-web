module.exports = {
    '{src,packages/domains}/**/translations/*.json': filenames => [`pnpm run translations:sort ${filenames.join(' ')}`],
    ...
    '{src,packages/domains}/**/*.{js,ts,tsx}': filenames => [`bash -c 'ESLINT_USE_FLAT_CONFIG=false pnpm exec eslint ${filenames.join(' ')}'`],
};
