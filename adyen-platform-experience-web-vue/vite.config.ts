...
export default defineConfig(({ mode }) => {
    ...
    return {
        plugins: [
            replaceCssClassPrefix(),
            pruneUnusedBentoIllustrations(),
            vue(),
            // VueI18nPlugin({
            //     /* options */
            //     // locale messages resource pre-compile option
            //     include: resolve(__dirname, './src/assets/translations/**'),
            // }),
            ...(isDevMode ? [vueDevTools()] : []),
            inlineComponentCssVariables(),
        ],
        resolve: {
            alias: {
        ...
        build: {
            ...
            rollupOptions: {
                external: ['vue', 'vue-i18n', /^@intlify\//, 'vue-router'],
                output: {
