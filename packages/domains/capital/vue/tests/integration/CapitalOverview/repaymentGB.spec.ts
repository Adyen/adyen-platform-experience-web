...
const STORY_ID = 'mocked-capital-capital-overview--repayment-gb';
...
test.describe('Repayment GB', () => {
    ...
    test.describe('Send repayment view', () => {
        ...
        test('should show repayment details after "Send repayment" button is clicked', async ({ page }) => {
            const copyIconButtons = getRepaymentModal(page).getByRole('button', { name: /^Copy (IBAN|account number|sort code|account owner)$/ });
            ...
            await Promise.all([
                ...
                expect(getRepaymentModal(page).getByText('IBAN')).toBeVisible(),
                expect(getRepaymentModal(page).getByText('GB01 ADYB 0123 4567 8901 23')).toBeVisible(),
                ...
                expect(getRepaymentModal(page).getByText('Sort code')).toBeVisible(),
                ...
                expect(getRepaymentModal(page).getByText('Adyen N.V. London Branch', { exact: true }).first()).toBeVisible(),
                expect(getRepaymentModal(page).getByText('Country/region')).toBeVisible(),
                expect(getRepaymentModal(page).getByText('GB', { exact: true })).toBeVisible(),
                ...
                expect(getRepaymentModal(page).getByText('Add Adyen N.V. London Branch as a payee using your bank’s website or app.')).toBeVisible(),
