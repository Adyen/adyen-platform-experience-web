...
import { ButtonVariant, ICapitalState, IDynamicOffersConfig, IGrantOfferResponseDTO } from '@integration-components/types';
...
export const CapitalOfferSelection = ({
    ...
    const isEarlyRenewal = useMemo(() => !!capitalState?.renewableGrants.length, [capitalState]);
    const renewableGrant = useMemo(() => capitalState?.renewableGrants[0], [capitalState]);

    const dynamicOffersConfig = useMemo(() => {
        ...
        return (
            ...
            ({
                ...
                minAmount: {
                    ...config?.minAmount,
                    value: isEarlyRenewal && renewableGrant ? renewableGrant.renewal?.minimumRenewalAmount?.value : config?.minAmount.value,
                },
            } as IDynamicOffersConfig)
        );
    }, [capitalState?.dynamicOffer, isEarlyRenewal, renewableGrant]);
    ...
    const highlightedFields = useMemo(() => {
        ...
        const existingGrantAmount = capitalState?.renewableGrants[0]?.remainingTotalAmount.value;
        ...
        return [
            {
                label: i18n.get('capital.offer.selection.earlyRenewal.newLoan'),
            ...
            {
                label: i18n.get('capital.offer.selection.earlyRenewal.existingLoanBalance'),
            ...
            {
                label: i18n.get('capital.offer.selection.earlyRenewal.amountYoullReceive'),
