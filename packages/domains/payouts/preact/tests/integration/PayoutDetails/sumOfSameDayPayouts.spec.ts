...
test.describe('Sum of same-day payouts', () => {
    test('should render payout details with "Sum of same-day payouts" tag', async ({ page }) => {
        ...
        await expect(page.getByText('Sum of same-day payouts', { exact: true })).toBeVisible();
        await expect(page.getByText('Sum of same-day payouts', { exact: true })).toHaveCount(1);
    });
});
