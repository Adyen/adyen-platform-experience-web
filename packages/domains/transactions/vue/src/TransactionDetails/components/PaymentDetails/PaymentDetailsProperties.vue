...
<template>
    <BentoStructuredList :class="TX_DATA_LIST">
        ...
        <BentoStructuredListItem v-for="item in customItems" :key="item.key" :label="i18n.get(item.key)">
            <BentoLink v-if="item.type === 'link' && item.config" :to="item.config.href" :target="item.config.target || '_blank'" isNotRouting>
                {{ item.value }}
            </BentoLink>
            <BentoTypography v-else variant="body">{{ item.value }}</BentoTypography>
        </BentoStructuredListItem>
    </BentoStructuredList>
</template>
