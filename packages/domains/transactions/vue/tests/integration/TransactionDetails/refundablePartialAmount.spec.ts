...
test.describe('Refundable - Partial amount', () => {
    ...
    test('should allow to refund partial payment amount', async ({ page }) => {
        ...
        const initialRefundButton = page.getByRole('button', { name: 'Refund €607.50', exact: true, disabled: false });

        await expect(amountInput).toBeVisible();
        await expect(amountInput).toHaveValue('607.50');
        await expect(initialRefundButton).toBeVisible();
        ...
        const refundButton = page.getByRole('button', { name: 'Refund €100.00', exact: true, disabled: false });
