...
export const downloadTransactions = async (
    ...
    const popover = page.getByRole('dialog').filter({ has: page.getByRole('button', { name: 'Download', exact: true }) });
...
export const openTransactionDetailsModal = async (page: Page, analyticsEvents: PageAnalyticsEvent[], transactionRowIndex = 0) => {
    ...
    const transactionType = await transactionRow
        .getByTestId('cell-transactionType')
        .textContent()
        .catch(() => 'Payment');
...
export const applyPspReferenceFilter = async (page: Page, analyticsEvents: PageAnalyticsEvent[]) => {
    const filterButton = page.getByRole('button', { name: /^PSP reference/ });
    const filterDialog = page.getByRole('dialog');
    const filterValue = await filterDialog.getByRole('textbox' /*, { name: 'PSP reference', exact: true }*/).inputValue();
    ...
    await expect(filterButton).toHaveText(new RegExp(`${filterValue.slice(0, 6)}`));
...
export const resetPspReferenceFilter = async (page: Page, analyticsEvents: PageAnalyticsEvent[]) => {
    ...
    await filterDialog.getByRole('button', { name: 'Clear', exact: true, disabled: false }).click();
...
export const setExactPspReference = async (page: Page, analyticsEvents: PageAnalyticsEvent[], exactPspReference: string) => {
    ...
    await filterDialog.getByRole('textbox' /*, { name: 'PSP reference', exact: true }*/).fill(exactPspReference);
...
export const selectSingleCategoryFromMultiSelectFilter = async (
    ...
    const filterButton = page.getByRole('button', { name: /^Type/ });
    const filterDialog = page.getByRole('dialog');
    ...
    await filterDialog.getByRole('checkbox', { name: category, exact: true, checked: false }).click();
    await filterDialog.getByRole('button', { name: 'Apply', exact: true, disabled: false }).click();
    await expect(filterDialog).toBeHidden();
...
export const selectSingleCurrencyFromMultiSelectFilter = async (page: Page, analyticsEvents: PageAnalyticsEvent[], currency: string) => {
    const filterButton = page.getByRole('button', { name: /^Currency/ });
    const filterDialog = page.getByRole('dialog');
    ...
    await filterDialog.getByRole('option', { name: currency, exact: true, selected: false }).click();
    await filterDialog.getByRole('button', { name: 'Apply', exact: true, disabled: false }).click();
    await expect(filterDialog).toBeHidden();
