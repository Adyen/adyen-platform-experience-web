...
// Drop selected stores that are no longer part of the store list, so the overview never
// fetches data for a store that is no longer available (the store filter is only
// rendered with more than one store).
watch(
