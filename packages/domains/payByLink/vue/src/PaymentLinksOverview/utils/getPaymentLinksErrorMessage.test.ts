...
describe('getPaymentLinksErrorMessage', () => {
    test.each([
        [ACCOUNT_MISCONFIGURATION, ['payByLink.common.errors.accountConfiguration', 'payByLink.common.errors.contactSupport']],
        [WRONG_STORE_IDS, ['payByLink.common.errors.storeID', 'payByLink.common.errors.contactSupport']],
    ...
    test.each(['00_500', undefined])('maps the %s error code to refresh guidance', errorCode => {
        ...
        expect(getPaymentLinksErrorMessage(error, ERROR_MESSAGE)).toMatchObject({
            messages: ['payByLink.overview.errors.couldNotLoadLinks', 'payByLink.common.errors.errorCodeSupport'],
            requestId: 'request-id',
    ...
    test('maps unknown errors to the configured message and contact-support action', () => {
        ...
        expect(getPaymentLinksErrorMessage(error, ERROR_MESSAGE, onContactSupport)).toMatchObject({
            messages: [ERROR_MESSAGE, 'payByLink.common.errors.errorCode'],
            requestId: 'request-id',
            onContactSupport,
            refreshComponent: true,
        });
