...
export const createAbortable = <T>(abortReason?: T) => {
    ...
    const _abort = () => {
        ...
        // To maintain the invariant that abortable is refreshable only if `_abortController` is `undefined`,
        // trigger the signal abort procedure only after setting `_abortController` to `undefined`. This
        // ensures that `_abortController` is already `undefined` before any signal abort listener gets a
        // chance to run. Think about an abort listener that triggers an abortable refresh when it runs —
        // that refresh will not happen unless `_abortController` is already set to `undefined`.
