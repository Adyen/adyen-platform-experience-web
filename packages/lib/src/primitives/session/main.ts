...
import { asPlainObject, enumerable, getter, isFunction, noop, parseTimestamp } from '@src/utils/common';
import { createAbortable } from '@src/primitives/common/abortable';
import { createEventEmitter } from '@src/primitives/common/eventEmitter';
import { createPromisor } from '@src/primitives/common/promisor';
import { UNSUBSCRIBE_TOKEN } from '@src/primitives/common/watchlist/constants';
...
const _RESOLVED_PROMISE = Promise.resolve();
...
export const createSessionHandle = <T, HttpParams extends any[] = any[]>(
    ...
    const _executeCreateNextSession = () => {
        if (_currentCreateNextSession !== _createNextSession) _sessionInitAbortable.abort();
    ...
    const _refreshSession = async () => {
        ...
        if (!_sessionInitializing) {
            ...
            if (_sessionInitEventPending) await _RESOLVED_PROMISE;
