...
export const createInterval = (callback: (time: DOMHighResTimeStamp) => any, ms = DEFAULT_INTERVAL_MS, runCallbackAsap = false) => {
    ...
    let _intervalDelay = Math.max(0, Math.min(~~ms, MAX_INTERVAL_MS));
    let _shouldRunCallbackAsap = (runCallbackAsap as any) === true;
    ...
    const _scheduleFrame = (time: number) => {
        ...
        if (!_shouldRunCallbackAsap) {
            ...
            const roundedElapsed = Math.ceil(elapsed / _intervalDelay) * _intervalDelay;
