...
describe('Button', () => {
    ...
    test('Uses a custom label when a status is defined', () => {
        ...
        expect(wrapper.find('.adyen-fp-spinner').length > 0).toBe(true);
    });

    test('Renders primary button as default', () => {
        const wrapper = getWrapper({});
        expect(wrapper.find('.adyen-fp-button').length).toBe(1);
        expect(wrapper.find('.adyen-fp-button--primary').length).toBe(0);
    });

    test('Renders secondary button', () => {
        const wrapper = getWrapper({ variant: 'secondary ' });
        expect(wrapper.find('.adyen-fp-button--secondary').length).toBe(1);
    });

    test('Renders action button', () => {
        const wrapper = getWrapper({ variant: 'action ' });
        expect(wrapper.find('.adyen-fp-button--action').length).toBe(1);
    });

    test('Renders ghost button', () => {
        const wrapper = getWrapper({ variant: 'ghost ' });
        expect(wrapper.find('.adyen-fp-button--ghost').length).toBe(1);
    });
});
