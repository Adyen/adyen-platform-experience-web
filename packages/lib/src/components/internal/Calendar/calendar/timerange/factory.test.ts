...
describe('createRangeTimestampsFactory', () => {
    ...
    const DAY_OFFSET = Object.freeze([0, 0, 1, 0, 0, 0, -1] as const);
    const DAY_OFFSET_INVERTED = Object.freeze(DAY_OFFSET.map(x => -x)) as typeof DAY_OFFSET;
    ...
    new Map([
        ...
        ['from timestamp and offset', { from: DAY_START, offset: DAY_OFFSET }],
        ['to timestamp and offset', { to: DAY_END, offset: DAY_OFFSET }],
    ...
    new Map([
        ...
        ['from timestamp and offset', { from: DAY_END, offset: DAY_OFFSET_INVERTED }],
        ['to timestamp and offset', { to: DAY_START, offset: DAY_OFFSET_INVERTED }],
    ]).forEach((config, description) => {
