...
export default function CalendarGrid({ calendar, trackToday, withSelected }: CalendarGridProps) {
    const today = useToday(trackToday);
    const cursorElementRef = useRef<HTMLElement | null>(null);
    const cursorRootProps = useCursorRoot(calendar, cursorElementRef, withSelected);

    return <ol className={'adyen-fp-calendar'} role="none" {...cursorRootProps}>{
