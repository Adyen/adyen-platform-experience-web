...
describe('applyBentoDomainOverrides', () => {
    ...
    test('uses a compiled domain translation when an override key exists', () => {
        ...
        applyBentoDomainOverrides(overrides, i18n, { 'bento.dataGrid.noResults': 'No results for {filter}' }, 'transactions');
        expect(overrides['bento.dataGrid.noResults']).toBe('No results for {filter}');
    });

    test('uses the component-specific domain key for Overview elements', () => {
        const overrides: Record<string, string> = {};
        const bentoKey = 'bento.dateRangePickerCalendar.lastMonth';
    ...
    test('uses each Overview component’s pagination key', () => {
        const overrides: Record<string, string> = {};
        const bentoKey = 'bento.dataGrid.pagination';
        const i18n = createI18n({
    ...
    test('rejects a missing component-specific domain key instead of using the SDK Bento fallback', () => {
        ...
        expect(() =>
            applyBentoDomainOverrides(
                ...
                {
                    'bento.dateRangePickerCalendar.customRange': 'Custom',
                    'bento.dateRangePickerCalendar.lastMonth': 'Last month',
                },
                'transactions',
                'transactions'
            )
    ...
    test('uses the general plural form when exact count variants are unavailable in Bento', () => {
        ...
        applyBentoDomainOverrides(
            overrides,
            i18n,
            { 'bento.fileUploaderRestrictions.maxUpload': '{count} result | {count} results' },
            'transactions'
        );

        expect(overrides['bento.fileUploaderRestrictions.maxUpload']).toBe('No results | {count} result | {count} results');
    });
    ...
    test('keeps the SDK Bento fallback when the target does not receive a plural count', () => {
        ...
        applyBentoDomainOverrides(overrides, i18n, { 'bento.dataGrid.noResults': '{count} result' }, 'transactions');
        expect(overrides['bento.dataGrid.noResults']).toBe('{count} result');
    });

    test('normalizes SDK Bento placeholders for Vue I18n', () => {
        expect(
            getBentoLocaleMessages(
                key => (key === 'bento.timelineItem.showMore' ? 'Show %{items} more' : null),
                key => key === 'bento.timelineItem.showMore'
            )
        ).toEqual({ 'bento.timelineItem.showMore': 'Show {items} more' });
    });

    test('keeps the file-size message for every configured Bento locale', async () => {
        for (const locale of getConfiguredLocales(await getI18nConfig())) {
            const localization = new Localization(locale, '', SDK_BENTO_TRANSLATION_SOURCES);
            ...
            ).toHaveProperty('bento.fileUploaderRestrictions.maxFileSize');
        }
    });
    ...
    test('uses the SDK English fallback for missing active-locale Bento translations', async () => {
        const localization = new Localization('da-DK', '', SDK_BENTO_TRANSLATION_SOURCES);
        await localization.ready;
