...
import { enumerable } from '../../../struct/property';
import { isNullish } from '../../../value/is';
import { createEffectStack } from './main';
import type { EffectStack } from './types';

describe('createEffectStack', () => {
    type TestEffect = {
        effect: EffectStack['effect'];
        stack: EffectStack;
    };
    ...
    beforeEach<TestEffect>(context => {
        ...
        Object.defineProperties(context, {
            ...
            stack: enumerable(createEffectStack(effect)),
        });
    });
    ...
    test<TestEffect>('should preserve the return value of the bottommost function in the call stack', ({ stack }) => {
        ...
        const boundFn3 = stack.bind((value?: any) => {
            ...
            return isNullish(value) ? value : boolValue;
        });

        expect(boundFn3()).toBeUndefined();
