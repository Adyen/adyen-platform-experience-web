...
If your extraction surfaces a *new* domain-agnostic customization type (e.g. for Vue-side customization or a new `OverviewDataCustomization` shape), promote it here in a separate commit before the bulk move. Do not leave duplicate copies in the domain layer.
...
- **Add a header comment** in the hoisted file documenting what was *not* hoisted and why.
...
- New shared packages introduced by the parallel branch (e.g. `@integration-components/composables-vue`, `@integration-components/core/vue`) — those are *its* PR's responsibility.
