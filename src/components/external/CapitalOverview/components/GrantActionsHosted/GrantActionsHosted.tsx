...
export const GrantActionsHosted: FunctionalComponent<GrantActionsHostedProps> = ({ className, expirationDate, missingActions }) => {
    ...
    const endpointsByAction = useMemo(
        () => ({
            signToS: signToSActionDetails,
            AnaCredit: anaCreditActionDetails,
        }),
        [anaCreditActionDetails, signToSActionDetails]
    );
    ...
    const actionMutation = useMutation({
        queryFn: (actionType: IMissingActionType) => {
            ...
            return endpointsByAction[actionType]?.(EMPTY_OBJECT, callbackQuery);
        },
        options: {
