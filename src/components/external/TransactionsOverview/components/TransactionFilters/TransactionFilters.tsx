...
const TransactionsFilters = () => {
    ...
    const { filters, balanceAccount, categories, currencies, createdDate, paymentPspReference } = transactionsFiltersResult;
    ...
    const initialFilters = useRef(filters.value);
    const initialBalanceAccount = useRef(balanceAccount.value);

    const resetFilters = useCallback(() => {
        resetBalanceAccountSelection();
        filters.reset({
            ...initialFilters.current,
            balanceAccount: initialBalanceAccount.current,
        });
    }, [filters.reset, resetBalanceAccountSelection]);

    useEffect(() => {
        initialBalanceAccount.current = undefined;
        resetBalanceAccountSelection();
    }, [balanceAccounts, resetBalanceAccountSelection]);

    useEffect(() => {
        if (!initialBalanceAccount.current) {
            initialBalanceAccount.current = balanceAccount.value;
        }
    }, [balanceAccount.value]);

    useEffect(() => {
        if (!initialFilters.current || filters.pristine) {
            initialFilters.current = filters.value;
        }
    }, [filters]);
