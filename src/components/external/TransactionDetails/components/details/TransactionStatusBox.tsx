...
const TransactionStatusBox = ({ transaction }: Pick<TransactionDataContentProps, 'transaction'>) => {
    ...
    const [paymentMethodType, paymentMethod] = useMemo(
        () => [getPaymentMethodTypeForTransaction(transaction), getDisplayablePaymentMethodForTransaction(transaction)] as const,
        [transaction]
    );
