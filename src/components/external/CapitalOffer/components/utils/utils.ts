...
export const getMaximumRepaymentDate = (days: number) => {
    return parseDate(dateStartUTCTimestampOffset(new Date(), days));
};

export const calculateRepaymentPeriodInMonths = (days?: number) => (days ? Math.ceil(days / 30) : null);
